# Principy konzultování — výukové sekce
...
### Requirement: Úplný seed pro všech deset principů

Počáteční data (seed) MUSÍ pro každý z deseti konzultačních principů obsahovat neprázdný český text v obou sekcích („Tipy a triky“ i „Příběhy z praxe“).

#### Scenario: Seed dokončen bez mezer

- **WHEN** se po migraci spustí idempotentní seed principů
- **THEN** všech deset principů má vyplněné oba textové bloky žádným prázdným řetězcem ani zástupným „Lorem ipsum“

### Requirement: Zobrazení výukových sekcí na stránce principů

Na stránce orientace s deseti principy konzultování systém MUSÍ u každého principu zobrazit pod stávajícím shrnutím dvě vizuálně a nadpisově oddělené sekce s přesnými štítky „Tipy a triky“ a „Příběhy z praxe“, naplněné obsahem z úložiště dat.

#### Scenario: Expert si prohlíží princip na desktopu

- **WHEN** autentizovaný expert otevře seznam principů orientace
